...
// --- SVG Icons ---
const CLOCK_ICON = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22c-5.523 0-10-4.477-10-10S6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/><path d="M15 10l-4 4 4 4"/><path d="M9 10v8"/></svg>`;
const BUILDING_ICON = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon"><path d="M18 20V6a2 2 0 0 0-2-2H8a2 2 0 0 0-2 2v14"/><path d="M2 20h20"/><path d="M14 12v.01"/></svg>`;
const ALERT_ICON = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon"><path d="m21.73 18-8-14a2 2 0 0 0-3.46 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><path d="M12 9v4"/><path d="M12 17h.01"/></svg>`;

// --- HTML Generation Helpers ---

/**
 * Generates the HTML for a single classroom card.
 * @param item - The data for the TV display item.
 * @returns An HTML string for the card.
 */
function generateCardHtml(item: TvDisplayInfo): string {
  const classroomHtml = item.classroomName
    ? `
  ...
  return `
    <div class="card">
      ...
      <div class="classroom-info">${classroomHtml}</div>
    </div>`;
}

/**
 * Generates the HTML for the "no classes" message.
 * @returns An HTML string for the placeholder card.
 */
function generateNoClassesHtml(): string {
  return `
    <div class="no-classes-card">
      ${ALERT_ICON.replace('class="icon"', 'class="icon-large"')}
...
/**
 * Generates the full HTML document for the TV panel.
 * @param displayData - The array of data to display.
 * @param publishedDate - The formatted date string for the header.
 * @returns The complete HTML document as a string.
 */
function getPanelHtml(displayData: TvDisplayInfo[], publishedDate: string): string {
  const mainContentHtml = displayData.length > 0
    ? displayData.map(generateCardHtml).join('')
  ...
  return `
    ...
    <body>
      <header>
        <h1>${CLOCK_ICON} Guia de Salas</h1>
        <p>${publishedDate}</p>
      </header>

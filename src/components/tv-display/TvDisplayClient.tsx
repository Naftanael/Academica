...
const DATA_CHECK_INTERVAL = 15 * 1000; // Check every 15 seconds
const RETRY_INTERVAL = 30 * 1000; // Retry connection after 30 seconds
const PAGE_RELOAD_INTERVAL = 6 * 60 * 60 * 1000; // Full page reload every 6 hours to prevent memory leaks
...
export default function TvDisplayClient({ initialDisplayData }: TvDisplayClientProps) {
  ...
  const checkDataChanges = React.useCallback(async function checkDataChanges() {
    ...
    try {
      ...
      // If the published timestamp is different, refresh the page to get new data from the server.
  ...
  // This effect ensures that when the server-side props change (after router.refresh()),
  // the client-side state is updated to match. This is crucial for the display to update correctly.
